...
public final class CameraManager {
    ...
    private final ConfiguringCamera mConfigManager;
     ...
     */
    private final CameraPreviewListener mCameraPreviewListener;
    ...
    private CameraManager(Context context) {
        this.mConfigManager = new ConfiguringCamera(context);
        mCameraPreviewListener = new CameraPreviewListener(mConfigManager);
        mFocus = new Focus();
    }
    ...
    public void openDriver(SurfaceHolder holder) throws IOException {
        if (mCamera == null) {
            ...
            if (!mInitialized) {
                mInitialized = true;
                mConfigManager.initCamera(mCamera);
            }
            mConfigManager.setParams(mCamera);
        }
    }
    ...
    public void stopPreview() {
        if (mCamera != null && mPreviewing) {
            mCamera.stopPreview();
            mCameraPreviewListener.setHandler(null, 0);
            mFocus.setFocusHandler(null, 0);
            mPreviewing = false;
        }
    }
    ...
    public void requestPreviewFrame(Handler handler, int message) {
        if (mCamera != null && mPreviewing) {
            mCameraPreviewListener.setHandler(handler, message);
            mCamera.setOneShotPreviewCallback(mCameraPreviewListener);
        }
    }
    ...
    public void requestAutoFocus(Handler handler, int message) {
        if (mCamera != null && mPreviewing) {
            mFocus.setFocusHandler(handler, message);
